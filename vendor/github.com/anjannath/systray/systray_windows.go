...
func (t *winTray) addBitMapMenuItem(bitmapPath string, item *MenuItem) error {
	...
	fmt.Println(bitmapPath, srcPtr, err)
	if err != nil {
		return err
	}
	res, _, err := pLoadImage.Call(
	...
	fmt.Println("LoadImage: ", res)
	if res == 0 {
		return err
	}

	h := windows.Handle(res)
	fmt.Println("BitmapHandle: ", h)
	res, _, err = pGetMenuItemID.Call(uintptr(t.menu), uintptr(item.id))
	fmt.Println("GetMenuID: ", res)
	fmt.Println(item.id)
	res, _, err = pSetMenuItemBitmaps.Call(
		uintptr(t.menu),
		uintptr(item.id),
		0,
		uintptr(h),
